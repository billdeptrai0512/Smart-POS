    ...
    return (
        <div className="bg-surface border border-border/60 rounded-[14px] px-4 py-3 flex items-center justify-between">
            <div className="flex flex-col">
                ...
                <span className="text-[11px] text-text-dim">Đơn vị: {getIngredientUnit(ingredient, storedUnit)}</span>
            </div>
            ...
            {isEditing ? (
                <div className="flex items-center gap-1.5">
                    ...
                    />
                    <span className="text-[12px] text-text-dim">đ / {getIngredientUnit(ingredient, storedUnit)}</span>
                </div>
            ) : (
                <span
