...
export default function ExpensePage() {
    ...
    // Unified form state
    const [expenseCategory, setExpenseCategory] = useState(safeInitialTab === 'fixed' ? 'fixed' : 'daily')
    const [costName, setCostName] = useState('')
    const [costAmount, setCostAmount] = useState('')
    const [isSubmitting, setIsSubmitting] = useState(false)
    const [deletingId, setDeletingId] = useState(null)

    // Sync form category with active tab when user switches tabs manually
    useEffect(() => {
        if (activeTab === 'fixed') {
            setExpenseCategory('fixed')
        } else if (expenseCategory === 'fixed') {
            setExpenseCategory('daily')
        }
    }, [activeTab])

    // Fixed cost edit state
    ...
    const submitUnifiedExpense = async () => {
        ...
        try {
            if (expenseCategory === 'fixed') {
                await handleAddFixedCost(costName.trim(), Number(costAmount) * 1000)
                setActiveTab('fixed')
            } else {
                const isRefill = expenseCategory === 'refill'
                const metadata = isRefill ? { free_form: true } : {}
                await handleAddExpense(costName.trim(), Number(costAmount) * 1000, isRefill, 'cash', metadata)
                setActiveTab('daily')
            }
            setCostName('')
            setCostAmount('')
    ...
    // activeExpenses shows both daily (!is_fixed && !is_refill) AND refill (free_form) now
    const dailyExpenses = (baseExpenses || []).filter(e => !e.is_fixed)
    const totalDaily = dailyExpenses.filter(e => !e.is_refill).reduce((sum, e) => sum + (e.amount || 0), 0)
            ...
            <header className="shrink-0 pt-6 pb-4 bg-surface border-b border-border/60 shadow-sm relative z-20 flex flex-col px-4 gap-3">
                <div className="flex items-center gap-2">
                    ...
                    <div className="flex flex-row gap-2 flex-1">
                        ...
                        >
                            <span className="text-[11px] font-black text-danger uppercase line-clamp-1">Nhật ký chi tiêu</span>
                            <span className="text-[11px] font-bold text-danger/80 leading-none mt-1 tabular-nums">Thực thu: -{formatVND(totalDaily)}</span>
                        </div>

                        {!isStaff && (
                            <div
                                onClick={() => { setActiveTab('fixed'); setCostName(''); setCostAmount('') }}
                            ...
                            >
                                <span className="text-[11px] font-black text-warning uppercase line-clamp-1">Cố định (P&L)</span>
                        ...
                        [...activeExpenses].sort((a, b) => new Date(b.created_at) - new Date(a.created_at)).map(expense => {
                            ...
                            return (
                                <div key={expense.id} className="bg-surface border border-border/60 rounded-[20px] p-4 shadow-sm flex flex-col gap-2 relative overflow-hidden opacity-90">
                                    <div className={`absolute top-0 right-0 text-[10px] font-black px-2 py-1 rounded-bl-[14px] uppercase tracking-wider ${isRefill ? 'bg-warning/10 text-warning' : 'bg-danger/10 text-danger'}`}>
                                        {isRefill ? 'Sau ca' : 'Trong ca'}
                                    </div>
                                    <div className="flex justify-between items-center mb-1">
                                        <div className="flex items-center gap-2 mt-1">
                                            <span className={`font-black text-[14px] ${isRefill ? 'text-warning' : 'text-danger'}`}>- {formatVND(expense.amount)}</span>
                                        </div>
                                    </div>
                ...
                <div className="p-4 bg-surface border-t border-border/60 flex flex-col gap-3">
                    <div className="flex gap-2">
                        <input
                            ...
                            className="flex-1 min-w-0 bg-surface-light border border-border/60 rounded-[12px] px-3 py-2 text-[14px] font-medium text-text placeholder:text-text-secondary/50 focus:outline-none transition-colors focus:border-primary/40"
                        />
                        <div className="relative shrink-0 flex items-center w-[125px] bg-surface-light border border-border/60 rounded-[12px] transition-colors overflow-hidden focus-within:border-primary/40">
                            <input
                                type="number"
                                placeholder="Số tiền..."
                                value={costAmount}
                                ...
                                onKeyDown={(e) => {
                                    if (e.key === 'Enter') submitUnifiedExpense()
                                }}
                            />
                            {costAmount && (
                    ...
                    <div className="flex gap-2 w-full p-1 bg-surface-light rounded-[12px]">
                        <button
                            onClick={() => setExpenseCategory('daily')}
                            className={`flex-1 py-2 rounded-[10px] uppercase text-[11px] font-bold transition-all ${expenseCategory === 'daily' ? 'bg-danger/80 text-white shadow-sm' : 'text-text-secondary hover:text-text'}`}
                        >
                            🔴 Trong ca
                        </button>
                        <button
                            onClick={() => setExpenseCategory('refill')}
                            className={`flex-1 py-2 rounded-[10px] uppercase text-[11px] font-bold transition-all ${expenseCategory === 'refill' ? 'bg-warning/80 text-white shadow-sm' : 'text-text-secondary hover:text-text'}`}
                        >
                            🟠 Sau ca
                        </button>
                        {!isStaff && (
                            <button
                                ...
                                className={`flex-1 py-2 rounded-[10px] uppercase text-[11px] font-bold transition-all ${expenseCategory === 'fixed' ? 'bg-text-secondary text-white shadow-sm' : 'text-text-secondary hover:text-text'}`}
                            >
                                🟡 Cố định
                            </button>
                        )}
                    ...
                    <button
                        onClick={submitUnifiedExpense}
                        disabled={!costAmount || isNaN(costAmount) || Number(costAmount) <= 0 || isSubmitting || !costName.trim()}
                        className={`w-full py-3 rounded-[12px] text-white text-[14px] font-black transition-colors disabled:opacity-50 disabled:cursor-not-allowed ${
                            expenseCategory === 'daily' ? 'bg-danger hover:bg-danger/90 active:bg-danger/80' : 
                            expenseCategory === 'refill' ? 'bg-warning hover:bg-warning/90 active:bg-warning/80' : 
                            'bg-text-secondary hover:bg-text-secondary/90 active:bg-text-secondary/80'
                        }`}
                    >
