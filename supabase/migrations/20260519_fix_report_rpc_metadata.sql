-- =============================================
-- Fix: report RPCs were missing `metadata` from expense SELECTs.
-- Without metadata, splitExpenses() can't tell free-form refills (after-shift
-- operational expenses, stored as is_refill=true + metadata.free_form=true)
-- from real NVL purchases (is_refill=true, no free_form). Result: every
-- after-shift "Vận hành" expense was being lumped into "Tồn kho" on range
-- reports and past-date daily reports. Today's daily report was unaffected
-- because it uses fetchTodayExpenses (direct query, already includes metadata).
--
-- Adds `metadata` to the 4 expense SELECT blocks in 3 RPCs.
-- =============================================
