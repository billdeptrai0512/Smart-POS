-- ==============================================================================================
-- get_ingredient_stocks_v2 — thêm 2 cờ warehouse_stock_set / counter_stock_set để phân biệt
-- "chưa từng nhập" với "đã nhập, giá trị thật = 0" — trước đây cả 2 case đều bị coalesce về 0
-- ở output nên nhìn giống hệt nhau, làm sai checklist onboarding "Nhập tồn kho/tồn quầy"
-- (đòi đủ 100% mới qua bước — 1 NVL tồn thật = 0 sẽ bị coi là "chưa nhập" mãi mãi).
--
-- warehouse_stock_set = true nếu NVL có ≥1 dòng trong refill_cte (đã "Đi chợ" HOẶC đã
-- "Hiệu chỉnh tồn kho" — cả 2 đều ghi expenses.is_refill=true, xem restockService.ts).
-- counter_stock_set   = true nếu NVL có remaining/opening khác-null — counter_cte vốn đã giữ
-- đúng null nội bộ, chỉ bị coalesce mất ở SELECT cuối cùng trước đây.
--
-- Đổi RETURNS TABLE (thêm cột) → Postgres không cho CREATE OR REPLACE, phải DROP + CREATE →
-- theo CLAUDE.md: khai lại SET search_path=public + REVOKE PUBLIC/anon + GRANT authenticated.
-- Toàn bộ logic tính current_stock/warehouse_stock/counter_stock giữ nguyên 100% so với
-- 20260718_ingredient_stocks_opening_fallback.sql — chỉ thêm 2 cột output mới.
-- ==============================================================================================
