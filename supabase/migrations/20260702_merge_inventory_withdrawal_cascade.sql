-- ============================================================
-- FIX tồn kho neo sai khi số "rút ra quầy" được nhập MUỘN
--
-- BUG THỰC TẾ (02/07): sáng 01/07 nhân viên rút bịch cuối (1026g) ra quầy nhưng
-- CHƯA gõ số "Lấy ra" vào form kiểm kê. 12:00 nhập kho 10260g → process_ingredient_restock
-- tính before_stock từ dữ liệu lúc đó (chưa thấy lượt rút) → lưu neo after_stock = 11286.
-- Chiều nhân viên mới gõ "Lấy ra 1026"; lượt rút mang created_at của PHIẾU CHỐT CA
-- (09:23 — TRƯỚC neo 12:00) nên get_ingredient_stocks_v2 / replay không bao giờ trừ nó
-- → lịch sử hiện "1026 → 11286" và kho tổng bị thổi phồng đúng 1026 vĩnh viễn.
--
-- FIX: merge_shift_closing_inventory — khi restock (số rút) của 1 NVL THAY ĐỔI so với
-- report hiện có, cascade TRỪ delta vào before_stock/after_stock của mọi phiếu refill
-- (chưa hủy, có after_stock) tạo SAU phiếu chốt ca. Đối xứng với 2 cascade đã có trong
-- 20260617 (backdate +qty, cancel −qty). Nhánh INSERT (phiếu chốt mới, created_at=now())
-- không thể có refill tạo sau nó → khỏi cascade.
--
-- Theo CLAUDE.md: giữ SET search_path = public; guard = SECURITY INVOKER + RLS
-- managers_shift_closings/managers_expenses (như bản gốc 20260619); re-REVOKE/GRANT.
-- ============================================================
