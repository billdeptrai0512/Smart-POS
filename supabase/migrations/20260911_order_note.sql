-- ==============================================================================================
-- 20260911_order_note.sql
-- Description: ghi chú riêng từng món (order_items.note) — nhân viên gõ ở modal Ghi chú của thanh
-- Tạo đơn (dine-in), in lên phiếu bếp + hiện ở chi tiết bàn. bulk_create_orders đọc thêm key
-- 'note' trong từng item của payload; client cũ không gửi key này → NULL, không vỡ gì.
--
-- Thân hàm giữ NGUYÊN VĂN từ 20260909_bulk_create_orders_scheduled_discount.sql, chỉ thêm cột
-- note vào INSERT order_items.
--
...
BEGIN
  ...
  LOOP
    ...
    LOOP
      ...
      -- note (20260911): rỗng → NULL như table_name; cắt 200 ký tự (khớp maxLength ô nhập ở CartNoteModal.jsx).
      INSERT INTO order_items (order_id, product_id, quantity, options, unit_cost, extra_ids, topping_ids, discount_amount, note)
      VALUES (new_order_id, v_product_id, v_quantity, v_options_text, ROUND(v_line_cogs)::INTEGER, v_extra_ids, v_topping_ids, COALESCE((item_rec->>'discount_amount')::INTEGER, 0),
              LEFT(NULLIF(TRIM(COALESCE(item_rec->>'note', '')), ''), 200));
    END LOOP;
