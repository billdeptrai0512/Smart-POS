-- ============================================================
-- Admin Dashboard v2 — vá 3 lỗ hổng phát hiện sau khi dùng bản đầu
-- (20260713_admin_dashboard_overview.sql) với dữ liệu thật:
--
-- 1. Dedupe theo address_id: trước đây 1 chi nhánh vừa "expiring" vừa
--    "inactive" tạo ra 2 dòng riêng trong `attention` → KPI "Cần chú ý"
--    đếm nhầm 2 chi nhánh trong khi thực chất là 1. Giờ mỗi chi nhánh chỉ
--    giữ 1 dòng (reason ưu tiên cao nhất).
-- 2. LIMIT 20 trước đây cắt luôn CẢ con số đếm (không chỉ cắt list hiển
--    thị) — khi > 20 chi nhánh cần chú ý, admin thấy số nhỏ hơn thật mà
--    không biết đã bị cắt. Giờ trả thêm `attention_total_count` /
--    `payment_issue_total_count` (COUNT không giới hạn, sau dedupe) tách
--    khỏi `attention` (vẫn top-20 để hiển thị).
-- 3. Chi nhánh đã từng trả tiền nhưng hết hạn không gia hạn (churned) biến
--    mất khỏi "cần chú ý" ngay khi rời khỏi "expiring" — nhóm dễ cứu nhất
--    (đã từng trả tiền thật) lại không có action nào follow-up. Thêm
--    reason 'churned_recent' (rời bỏ trong 30 ngày gần đây).
-- 4. "inactive" trước đây chỉ áp cho paid_addresses — trial đang chạy mà
--    chưa từng phát sinh đơn/ca nào thì phải đợi tới ≤3 ngày cuối
--    (trial_ending) mới bị flag, mất cửa sổ gọi hỗ trợ chuyển đổi sớm.
--    Thêm reason 'trial_inactive' (trial đã bắt đầu ≥2 ngày, chưa từng
--    hoạt động).
--
-- Vẫn 1 RPC duy nhất, read-only, an toàn chạy lại (CREATE OR REPLACE).
-- Không đổi signature → không cần REVOKE/GRANT lại, nhưng giữ nguyên cho
-- nhất quán với các migration khác.
-- ============================================================
