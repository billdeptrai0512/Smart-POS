-- ============================================================
-- Admin Dashboard v4 — vá lỗ mù lớn nhất trong phễu: trial KHÔNG bắt đầu lúc
-- đăng ký, chỉ bắt đầu ở ca chốt FULL đầu tiên (xem docs/MONETIZATION.md §1,
-- trigger grant_trial_on_first_full_shift_close). Trước mốc đó, địa chỉ có
-- 0 dòng address_subscriptions — không paid, không trial, không churned —
-- vô hình với mọi metric admin_dashboard_overview đang trả. Đây chính là
-- khúc quyết định "onboarding hiệu quả → trial tăng" theo giả thuyết đã
-- thống nhất, và hiện KHÔNG được đo ở đâu cả.
--
-- Thêm:
-- 1. never_activated: địa chỉ tạo ≥3 ngày trước, 0 dòng subscription. Đưa vào
--    attention list (reason mới, ưu tiên THẤP NHẤT — xa tiền thật nhất, xếp
--    sau trial_inactive theo đúng thứ tự ưu tiên hiện có) + owner_phone để gọi.
-- 2. pending_activation_count: cùng điều kiện 0 dòng nhưng còn trong 3 ngày
--    đầu (chưa đáng lo, chỉ hiển thị context).
-- 3. activated_count: địa chỉ đã từng có ≥1 dòng subscription (đã qua được
--    cửa ải chốt ca full ít nhất 1 lần, bất kể sau đó paid/trial/churned).
--
-- 3 số này expose trong `subscription` để OnboardingFunnelCard nối thêm 1
-- khối "Sau đăng ký → chốt ca full lần đầu" — dùng address_id thật, KHÔNG
-- trộn vào mảng `stages` (đó là guest funnel theo visitor_id ẩn danh, khác
-- thực thể).
--
...
BEGIN
    ...
    activated_calc AS (
        SELECT COUNT(DISTINCT address_id) AS n FROM address_subscriptions
    ),
    attention_raw AS (
        SELECT
    ...
    SELECT jsonb_build_object(
        ...
        'subscription', jsonb_build_object(
            ...
            'activated_count', (SELECT n FROM activated_calc),
            'pending_activation_count', (SELECT COUNT(*) FROM unactivated_addresses WHERE NOT is_stuck),
            'never_activated_count', (SELECT COUNT(*) FROM unactivated_addresses WHERE is_stuck)
        ),
