-- ==============================================================================================
-- 20260916_bulk_create_orders_program_discount_recorded.sql
-- Description: giảm giá theo chương trình (discount_programs) được GHI LẠI thành discount_amount
-- thay vì âm thầm hạ giá bán. 20260909 thay thẳng v_unit_price bằng giá chương trình → đơn ghi
-- xuống có total đã giảm nhưng discount_amount = 0: thẻ Doanh thu của báo cáo ngày hiện "Giảm
-- giá 0đ" dù quán đang chạy chương trình, còn bill in thì in giá GỐC ở từng dòng (giá bán không
-- lưu theo dòng, dựng lại từ products.price — xem utils/billLines.js) mà tổng lại là giá đã
-- giảm, chênh nhau không có dòng nào giải thích.
--
-- Từ đây: tiền hàng (v_order_total) cộng theo GIÁ GỐC, phần chênh của chương trình cộng vào
-- discount_amount của đơn LẪN của dòng (order_items.discount_amount, cùng chỗ với giảm giá bấm
-- tay). orders.total KHÔNG đổi một đồng nào — tổng gốc trừ giảm ra đúng số cũ.
--
-- Client gửi discount_amount chỉ gồm phần bấm tay (giỏ hàng đã mang sẵn giá chương trình ở
-- basePrice, xem POSContext.doSubmit) — server cộng thêm phần chương trình ở đây, không có
-- đường nào cộng hai lần.
--
-- Đồng thời lọc chương trình theo new_order_time (giờ BÁN) thay vì now() (giờ GHI). Đơn offline
-- sync trễ — mất mạng lúc 15h trong giờ chương trình, máy gửi lại lúc 19h qua bulkSubmitOrders —
-- trước đây bị tính lại theo giá gốc: khách trả 10k mà doanh thu ghi 22k, và ngược lại đơn bán
-- ngoài giờ chương trình mà sync trúng giờ thì mất tiền. created_at do client gửi (hàng orders
-- vốn đã tin số này) nên máy sai đồng hồ lấy được giá của ngày khác — đổi lại là không tính sai
-- tiền mỗi lần mất mạng.
--
-- KHÔNG backfill đơn cũ: đơn trước migration này giữ nguyên discount_amount = 0, báo cáo ngày cũ
-- không đổi số.
--
...
--
-- Thân hàm giữ NGUYÊN VĂN từ 20260909_bulk_create_orders_scheduled_discount.sql, chỉ đổi khối
-- resolve giá chương trình (không ghi đè v_unit_price nữa) + 2 chỗ cộng discount.
--
...
DECLARE
  ...
  v_program_line INTEGER;   -- tiền chương trình giảm cho DÒNG hiện tại (đã nhân số lượng)
  v_program_total INTEGER;  -- cộng dồn phần chương trình giảm của cả đơn
  ...
  LOOP
    ...
    LOOP
      ...
      WHERE dpp.product_id = v_product_id
        ...
        AND dp.enabled
        AND (dp.start_date IS NULL OR (new_order_time AT TIME ZONE 'Asia/Ho_Chi_Minh')::date >= dp.start_date)
        AND (dp.end_date IS NULL OR (new_order_time AT TIME ZONE 'Asia/Ho_Chi_Minh')::date <= dp.end_date)
        AND (
          dp.days_of_week = '{}'
          OR EXTRACT(DOW FROM new_order_time AT TIME ZONE 'Asia/Ho_Chi_Minh')::SMALLINT = ANY(dp.days_of_week)
        );

      -- Giá bán giữ nguyên giá gốc; phần chương trình giảm thành discount_amount (xem đầu file).
      -- GREATEST(...,0): chương trình 'fixed' đặt đồng giá CAO hơn giá gốc thì coi như không
      -- giảm, không được cộng tiền vào đơn bằng "giảm giá" âm.
      v_program_line := CASE
        WHEN v_discount_price IS NULL THEN 0
        ELSE GREATEST(v_unit_price - v_discount_price, 0) * v_quantity
      END;
      v_program_total := v_program_total + v_program_line;
      ...
      -- cấp đơn (client-declared, xem comment ở migration đó). Cộng thêm phần chương trình giảm
      -- của dòng này: bill in cần biết dòng nào được giảm để in giá gạch ngang.
      INSERT INTO order_items (order_id, product_id, quantity, options, unit_cost, extra_ids, topping_ids, discount_amount)
      VALUES (new_order_id, v_product_id, v_quantity, v_options_text, ROUND(v_line_cogs)::INTEGER, v_extra_ids, v_topping_ids, COALESCE((item_rec->>'discount_amount')::INTEGER, 0) + v_program_line);
    END LOOP;
    ...
    v_discount_capped := LEAST(GREATEST(v_discount_amount, 0) + v_program_total, v_order_total);
