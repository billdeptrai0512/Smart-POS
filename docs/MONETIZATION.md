...
**✅ AS-BUILT (2026-08-13, bản 2) — watermark, nhịp xuống 1.5s.** Bản đầu poll 5s và mỗi nhịp tải về đầu đơn **cả ngày** chỉ để so. Chi phí = `số nhịp × số đơn`, nên quán càng đông thì payload càng to *mà* càng muốn nhịp nhanh — không mua được độ trễ bằng cách hạ khoảng nhịp. Đo thật (gzip, 12h, 200 đơn/ngày/máy): 5s → 28 MB, 2s → 69 MB, 1s → 137 MB.

`20260813_orders_sync_marks.sql` thêm bảng `order_sync_marks(address_id, rev)` do trigger trên `orders` nuôi, cùng RPC `orders_sync(p_address_id, p_rev)`: client gửi kèm `rev` đang giữ — khớp thì trả đúng con số đó và **không đọc `orders` dòng nào** (~80 byte), lệch mới trả mảng đầu đơn. Chi phí thành O(1) theo số đơn ⇒ nhịp **1.5s tốn ~9 MB/máy/ngày, rẻ hơn cả nhịp 5s cũ**, độ trễ điển hình ~1s. Gỡ luôn cơ chế giãn nhịp 15s lúc vắng: nó sinh ra để né chi phí của nhịp nhanh, mà chi phí đó không còn.
