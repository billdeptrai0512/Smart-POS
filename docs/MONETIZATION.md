...
|---|---|---|
| Có gói `all`, còn > 3 ngày | `Trọn bộ · còn 23 ngày` | text-dim |
| Có gói `all`, còn ≤ 3 ngày | `còn 2 ngày — Gia hạn` | text-warning |
| Hết hạn / chưa mua | `Mở khoá báo cáo` | text-primary, mời chào |
...
**✅ AS-BUILT (2026-06-10):** thực tế **shipped bằng realtime** (`usePaymentListener` nghe INSERT `address_subscriptions`), KHÔNG phải poll-while-pending. Đang chạy thật ở quy mô soft-launch — đủ tốt. Phân tích bên trên giữ làm **kế hoạch migrate** nếu/khi chạm trần realtime (quota conn / per-event auth ở ~10k đồng thời). Khi đó đổi listener sang poll-while-pending — backend (c) không phải đụng.
...
- **Xác nhận về client = realtime** (`usePaymentListener` nghe INSERT `address_subscriptions`) — xem §7.1.

**Đã có sẵn (cứu rủi ro mất tiền thật):** server-side kill switch đọc `app_config` (flip không cần redeploy); idempotency theo `sepay_tx_id`; `manual_review` cho lệch tiền. **Còn thiếu:** trang dashboard đối soát thủ công (list intent `pending`/`manual_review` vs sao kê) — xem §10 Bước 11.
...
| 10 | **AS-BUILT: xác nhận thanh toán = realtime** (`usePaymentListener`); poll-while-pending để dành làm bản migrate khi chạm trần scale (§7.1) | Realtime đang chạy thật ở soft-launch; đổi sang poll khi cần ~10k đồng thời, backend không đụng |
...
*Cập nhật lần đầu: 2026-05-10. Sửa lớn 2026-06-03: tier basic/pro → 3 module độc lập (cashflow/inventory/finance) + giá năm + bundle + all-branches. 2026-06-20: đồng bộ as-built payment backend LIVE (SePay webhook + confirm_payment + QR động + realtime, §7) + thêm §11 Referral. Sửa khi mô hình thay đổi — KHÔNG để doc này lệch với code.*
